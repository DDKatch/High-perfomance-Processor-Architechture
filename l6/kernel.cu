...
#define ROW_SIZE 1024 // Matrix size = ROW_SIZE * ROW_SIZE
...
__global__ void matrixMulKernel(int* matr1, int* matr2, int* matr3, int* matr4){
	__shared__ int t_matr2[(ROW_THREADS + 1) * ROW_THREADS];
	__shared__ int t_matr4[(ROW_THREADS + 1) * ROW_THREADS];
	...
	if (x < ROW_SIZE && y < ROW_SIZE){
		...
		t_matr2[threadIdx.x * (ROW_THREADS + 1) + threadIdx.y] = matr2[offset];
		t_matr4[threadIdx.x * (ROW_THREADS + 1) + threadIdx.y] = matr4[offset];
	}
	...
	if (x < ROW_SIZE && y < ROW_SIZE){
		int offset = y * ROW_SIZE + x;
		matr1[offset] = matr1[offset] * t_matr2[threadIdx.y * (ROW_THREADS + 1) + threadIdx.x] + matr3[offset] * t_matr4[threadIdx.y * (ROW_THREADS + 1) + threadIdx.x];
	}
}
...
int* matrixMulAndSumCuda(int* matrix){
	...
	dim3 threadsPerBlock(ROW_THREADS , ROW_THREADS);
	dim3 numBlocks((ROW_SIZE + ROW_THREADS - 1) / threadsPerBlock.x, (ROW_SIZE + ROW_THREADS - 1) / threadsPerBlock.y);
